...
def binary_search(arr, x):
    ...
    while low <= high:
        mid = (low + high)//2   #검색 범위 중간값

        if arr[mid] < x:    #중간점의 값이 찾고자 하는 값보다 작을 때
            low = mid + 1   #검색 범위의 시작점을 중간점 다음 인덱스로 설정
        elif arr[mid] > x:  #중간점의 값이 찾고자 하는 값보다 클 때
            high = mid - 1  #검색 범위의 끝점을 중간점 이전 인텍스로 설정
        else:
            return mid
